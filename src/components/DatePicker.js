  ...
  _create(source) {
    ...
    let template = `
      <div className="select_wrap">
        <ul className="select_options" style="transform: translate3d(0, 0, ${-this.radius}px) rotateX(0deg);">
          {{circleListHTML}}
          <!-- <li className="select_option">a0</li> -->
        </ul>
        <div className="highlight">
          <ul className="highlight_list">
            <!-- <li className="highlight_item"></li> -->
            {{highListHTML}}
          </ul>
        </div>
      </div>
    ...
    for (let i = 0; i < source.length; i++) {
      circleListHTML += `<li className="select_option"
                    style="
    ...
    let highListHTML = '';
    for (let i = 0; i < source.length; i++) {
      highListHTML += `<li className="highlight_item" style="height: ${this.itemHeight}px;">
                        ${source[i].text}
                      </li>`
    }
    ...
    if (this.options.type === 'infinite') {
      ...
      highListHTML += `<li className="highlight_item" style="height: ${this.itemHeight}px;">${source[0].text}</li>`
    }

    this.elems.el.innerHTML = template
                                .replace('{{circleListHTML}}', circleListHTML)
                                .replace('{{highListHTML}}', highListHTML);
    this.elems.circleList = this.elems.el.querySelector('.select_options');
    this.elems.circleItems = this.elems.el.querySelectorAll('.select_option');


    this.elems.highlight = this.elems.el.querySelector('.highlight');
    this.elems.highlightList = this.elems.el.querySelector('.highlight_list');
    this.elems.highlightitems = this.elems.el.querySelectorAll('.highlight_item');
