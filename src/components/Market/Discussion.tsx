    ...
    return (
        <Stack>
            <HStack alignItems={'flex-start'}>
                <Image src={'/profile-photo.png'} objectFit={'cover'} borderRadius={'full'} boxSize={'35px'} alt={'Profile Photo'} />
...
export const DiscussionList = () => {
    ...
    useEffect(() => {
        connection.getProgramAccounts(new web3.PublicKey(DISCUSSION_PROGRAM_ID)).then(async (accounts) => {
            const discussions: Discussion[] = accounts.reduce((accum: Discussion[], { pubkey, account }) => {
    ...
    return (
        <div>
            {
                discussions.map((discussion, i) => <Card key={i} discussion={discussion} /> )
            }
        </div>
    )
}